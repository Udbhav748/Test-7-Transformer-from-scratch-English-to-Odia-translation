...
from src.model.scaled_transformer import (
    ...
    ScaledEmbeddings,
    ScaledFeedForward,
    ScaledMultiHeadAttention,
    ScaledPositionalEncoding,
    ScaledPreLNDecoderBlock,
    ScaledPreLNEncoderBlock,
)
...
with tab_benchmarks:
    ...
    chrf_score = EVAL_RESULTS.get("chrf_score") if EVAL_RESULTS else None
    ...
    with m2:
        chrf_display = f"{chrf_score:.2f}" if chrf_score is not None else "n/a"
        st.markdown(
            f"""
            <div class="kpi-container">
                <div class="kpi-title">Translation Quality (chrF++)</div>
                <div class="kpi-value">{chrf_display}</div>
                <div class="kpi-subtext neutral">Character-level quality score (0-100)</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
