...
Every figure in this section is a **real, executed output of
[`notebooks/model_parameters_and_results.ipynb`](notebooks/model_parameters_and_results.ipynb)**
— generated by running the notebook's actual code cells against the real checkpoints, tokenizers,
and result files in this repo, not drawn or mocked up separately. Running the notebook yourself
(`jupyter nbconvert --to notebook --execute notebooks/model_parameters_and_results.ipynb`)
reproduces these exact charts.

While fixing a bug for this pass — the notebook's attention-heatmap cell was reading the wrong
JSON key (`attention_weights` instead of the actual `attention_matrix`) and displaying raw,
undecoded byte-level BPE token pieces as axis labels — both were fixed
(`scripts/run_attention_demo.py` now decodes each token id properly instead of using the
tokenizer's raw `.tokens`) and the notebook re-run for real.
...
### Parameter breakdown by layer, baseline vs. scaled
